...
class CNNBLSTM:
	...
	gpu_memory = tf.GPUOptions(per_process_gpu_memory_fraction=0.25)
    ...
    def predict(self,input_data,sequence_length,weightfile,dbfile,max_target_length,sampleids):
	    ...
	    #calculating Word Level Accuracy
            nb_words,corr_words =0,0
            for i in range(len(actual_targets)):
                nb_words = nb_words + len(actual_targets[i].encode('utf-8').replace("***","").split("*"))

            print("Total number of words: "+str(nb_words))

            for i in range(len(actual_targets)):
                actual_words = actual_targets[i].encode('utf-8').replace("***","").split("*")
                predicted_targets = predicted_words[i].encode('utf-8').replace("***","").split("*")

                for j in range(len(actual_words)) :
                    if(actual_words[j]==predicted_targets[j]) :
                        corr_words+=1

            word_acc = (corr_words/float(nb_words))*100
...
'''
def main(task,mode,dbfile,data_file,encoded_file,weightfile,batchsize):
    ...
    [x_train, x_test], nb_classes, seqlen, [train_y,test_y], max_target_length, max_seq_length, max_h, char_int, transcription_length,sampleids = load_data(
        data_file,encoded_file, batchsize, 100,generate_char_table)
    nb_features = 1
    x = [x_train, x_test]
    y = [train_y, test_y]
...
dir = "/home/cvpr/Documents/Shahil/DegradedOCR-master"
...
if (task == "Predict"):
    batchsize = 100 #Number of predictions


main(task,mode,dbfile,data_file,Encoded_gt,weightfile,batchsize)
